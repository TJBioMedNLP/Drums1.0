...
printf OUT "pmid\torganism\tmut_pat1\tpos_patt\twtaa\tmtaa\tpos\tgenes\ttype\tfasta_check\tgi\tgene_name\tprot_id\n";
...
while(<IN>)
{
	...
	my ($pmid, $org, $mutpat, $pospat, $wtaa, $mtaa, $position, $genepat, @others) = split/\t/;
	
    unless ($pmid =~ /\d+/) #Skips rows with non_digit values (intended to skip the header)
    {
		next;
    }
    ($wtaa, $mtaa) = mutation_conversion($wtaa, $mtaa);
	...
	my @tojoin = ($pmid, $org, $mutpat, $pospat, uc $wtaa, uc $mtaa, $position, $genepat, $col11, $col12, $col13, $col14, $col15);
