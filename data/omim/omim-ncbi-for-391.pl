...
my $offsetRescue = -1;
print 'Would you like to use signal peptide offset for unmapped OMIM AVs:[Y/N]';
$choice = $ARGV[4];
if($sbstep > 0)
{
	$choice = <>;
...
SKIPID:foreach my $id (@omim_list)
{

	my $fseq_ex = "$path/tmp/$id.uniprot";
	if(open(INFILE, $fseq_ex))
	{
		close(INFILE);
		print "skip: $id\n";
		next SKIPID;
...
=cut	
	...
	for(my $i=0; $i< $refseq_number; $i++)#bug 20090515
	{
		...
		if($type == 4)
		{
			print "\nThe mapped sequence id is: ". $seqs[$i][2];
			print "\n";
		}
	}
=head	
...
=cut
	...
	##########
	###
	my $uname = &GetUniqueFileName();
	if($option == 5)
	{
		my $seqfile = "$path/tmp/$id.uniprot";
...
=head
		#for contig view
		my $MergedSeqfile = "$path/tmp/$uname.merged";
		my $PrimarySeqfile = "$path/tmp/$uname.primary";
		...
		my $align_outfile = "$path/tmp/$uname.align_rlt";
...
sub GetAVlist()
{
	...
	my $sql = qq{select position, aa  from point_mutation where omim_id = "$omim_id";};
